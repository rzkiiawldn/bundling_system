...
class Reports extends CI_Controller
{
  ...
  public function report_request_bundling($id_location = null, $id_client = null)
  {
    ...
    if ($id1 != null and empty($id2)) {
      $item = $this->db->query("SELECT * FROM request_bundling AS rb JOIN item_bundling AS ib ON rb.id_item_bundling = ib.id_item_bundling JOIN status ON rb.id_status = status.id_status WHERE rb.id_location = $id1  AND report = 1")->result_array();
    } elseif ($id2 != null) {
      $item = $this->db->query("SELECT * FROM request_bundling AS rb JOIN item_bundling AS ib ON rb.id_item_bundling = ib.id_item_bundling JOIN status ON rb.id_status = status.id_status WHERE rb.id_location = $id1 AND rb.id_client = $id2  AND report = 1")->result_array();
    } else {
  ...
  public function news_bundling_report($id_location = null, $id_client = null)
  {
    ...
    if ($id1 != null and empty($id2)) {
      $item = $this->db->query("SELECT * FROM news WHERE id_location = $id1")->result_array();
    } elseif ($id2 != null) {
      $item = $this->db->query("SELECT * FROM news WHERE id_location = $id1 AND id_client = $id2")->result_array();
    } else {
