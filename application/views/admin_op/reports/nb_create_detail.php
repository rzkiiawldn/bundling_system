          ...
          <div class="card card-info shadow">
            ...
            <div class="card-body">
              ...
              <?php if (!empty($this->uri->segment(5))) { ?>
                <form action="<?= base_url('admin_op/reports/add_item/' . $this->uri->segment(4)); ?>" method="post">
                <?php } else { ?>
                  <form method="post" action="<?= base_url('admin_op/reports/add_item') ?>">
                  <?php } ?>
                  <div class="row">
                      ...
                      <tbody>
                        ...
                        foreach ($news_detail as $row) : ?>
                          <tr>
                            ...
                            <td>
                              <?php if (!empty($this->uri->segment(5))) { ?>
                                <form action="<?= base_url('admin_op/reports/delete_item_satuann/' . $this->uri->segment(4) . '/' . $row['id_news_detail']); ?>" method="post">
                              ...
                              <?php } else { ?>
                                <form action="<?= base_url('admin_op/reports/delete_item_satuann/' . $row['id_news_detail']); ?>" method="post">
                                  <input type="hidden" name="id_news_detail" value="<?= $row['id_news_detail'] ?>">
                                  <input type="hidden" name="id_news" value="<?= $row['id_news'] ?>">
                                  <button type="submit" class="btn btn-sm btn-danger" title="hapus" onclick="return confirm('Delete ?')"><i class="fas fa-trash"></i></button>
                                </form>
